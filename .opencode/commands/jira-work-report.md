...
4. Read `.jira/issue_templates/jira-work-report.md`, build the implementation report from recent commits on the current branch according to the skill rules, and post it as a comment on the derived Jira issue using MCP server `jira` tools only (do not use Jira CLI or local scripts).