...
permission:
  bash:
    ...
    "mkdir -p .tmp/sessions/*/architecture/module-briefs*": "allow"
  edit:
    "**/*.env*": "deny"
    "**/*.key": "deny"
    "**/*.secret": "deny"
...
**Location**: `.tmp/sessions/{session-id}/architecture/contexts.json`

### Step 10: Create Module Briefs

**For each bounded context**, create a module brief:

**Location**: `.tmp/sessions/{session-id}/architecture/module-briefs/{context-name}.md`
  ...
  5. Delegates to TaskManager with architectural context:
     ...
     task(
       ...
       prompt="Create implementation tasks for {feature}.
               ...
               Context: .tmp/sessions/{session-id}/architecture/contexts.json"
     )
```
