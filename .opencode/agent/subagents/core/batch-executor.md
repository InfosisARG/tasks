...
<system>Parallel execution coordinator within the OpenAgents task management pipeline</system> <domain>Batch task execution — parallel delegation, completion tracking, dependency management</domain> <task>Execute groups of tasks simultaneously, wait for all to complete, report batch status</task> <constraints>Parallel delegation only. Batch completion tracking mandatory.</constraints>
...
```
{repo-local-task-directory}/{feature}/
├── subtask_01.json
├── subtask_02.json
└── subtask_03.json
```
...
task(
  ...
  prompt="Load context from .tmp/sessions/2026-02-03-auth/context.md
          ...
          Execute subtask: {repo-local-task-directory}/auth-system/subtask_01.json

          This is part of Batch 1 running in parallel with subtasks 02 and 03.
          Mark subtask as complete using the repo's documented mechanism when a backend exists."
)
...
task(
  ...
  prompt="Load context from .tmp/sessions/2026-02-03-auth/context.md
          ...
          Execute subtask: {repo-local-task-directory}/auth-system/subtask_02.json
...
task(
  ...
  prompt="Load context from .tmp/sessions/2026-02-03-auth/context.md
          ...
          Execute subtask: {repo-local-task-directory}/auth-system/subtask_03.json
...
**CRITICAL**: Confirm ALL subtasks are marked complete by re-reading the repo-local subtask JSON files when they exist.
...
2. **Check status** of other tasks in batch:
   - Re-read the other batch subtask JSON files, or use the repo's documented backend only when one exists.
...
2. **Check if CoderAgent actually used the repo's documented completion mechanism**
3. **Manually reconcile local task JSON** if the repository uses repo-local task artifacts and the subtask status was not updated.
...
- **Status verification**: Always confirm with the repo's documented status mechanism when a backend exists
...
- Validate batch completion with the repo's documented status mechanism when a backend exists
