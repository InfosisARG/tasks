...
- Repo-local task artifacts only when this repository defines an active backend
...
- Task artifacts validated only when a repo-local backend documents how to do so
...
2. Initialize stage tracking only if this repository documents a real stage backend. Otherwise track stages in-session and report that persistence is not active.
...
1. **Validate Prerequisites**
   - Use the repo's documented stage validation mechanism when one exists.
   - Otherwise validate prerequisites in-session only.
...
3. **Validate Stage Completion**
   - Use the repo's documented stage completion mechanism when one exists.
   - Otherwise report completion in-session only.
...
3. **Execute Rollback (if needed)**
   - Use the repo's documented rollback mechanism when one exists.
   - Otherwise report the rollback steps needed and stop for approval.
...
1. **Check Stage Status**
   - Use the repo's documented stage status mechanism when one exists.
   - Otherwise summarize current stage progress in-session.
...
2. **Execute Abort**
   - Use the repo's documented abort mechanism when one exists.
...
task(
  ...
  prompt="Create task breakdown for {feature}.
         ...
         Output: task breakdown summary, plus repo-local task artifacts only when an active backend is documented"
)
```
