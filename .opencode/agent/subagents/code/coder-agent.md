...
> **Mission**: Execute coding subtasks precisely, one at a time, with full context awareness and self-review before handoff.
  ...
  <system>Subtask execution engine within the repository task-management pipeline when one is available</system>
  ...
  <constraints>Sequential execution. Self-review mandatory before handoff.</constraints>
...
```
Location: use the repo-local subtask path defined by the active task workflow
```
...
- If the repository documents a real task backend, use its documented completion mechanism.
- Otherwise, edit the subtask JSON directly and replace the in-progress status block with:
...
"completion_summary": "Implemented deliverable and recorded completion"
```

**8.2 Verify Status Update**:

- Re-read the updated subtask JSON and confirm it now shows `completed`.
- If the parent `task.json` exists and tracks counters, update `completed_count` only when the repository workflow expects local JSON tracking.
- If no documented backend exists and no local JSON task artifacts exist, report completion in-response only.
...
- Confirmation that subtask status is marked complete or that completion was reported in-response when no backend exists
